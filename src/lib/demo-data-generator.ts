...
export async function generateDemoDataset(progressCallback: (msg: string) => void) {
  ...
  for (const state of mexicanStates) {
    ...
    for (let i = 1; i <= storesPerState; i++) {
      const storeId = `${state.slice(0,3).toUpperCase()}-${i.toString().padStart(3, '0')}`;
  ...
  // 4. Devices (approx 5,000 for demo performance)
  ...
  for (let i = 0; i < 5000; i++) {
    ...
    currentBatch.set(ref, {
      ...
      name: `TV ${i % 10 + 1}`,
      storeId,
      regionId: DEMO_REGIONS.find(r => r.states.some(s => storeId.startsWith(s.slice(0,3).toUpperCase())))?.id || 'centro',
